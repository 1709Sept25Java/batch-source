...
RETURN VARCHAR2             --RETURN A VARCHAR2 WITH THE TIME
...
BEGIN --CALL TIME FUNCTION AND PRINT TO DBMS OUTPUT
    DBMS_OUTPUT.PUT_LINE('TIME: '||CURR_TIME()); 
END;

--create a function that returns the length of name in MEDIATYPE table
CREATE OR REPLACE FUNCTION NAME_LENGTH (MT_ID IN NUMBER)   --IN PARAMETER ID OF THE MEDIATYPE YOU WANT TO FIND THE NAME OF
RETURN NUMBER
IS MT_NAME_LENGTH NUMBER;  --RETURN THE NAME LENGTH
BEGIN
    SELECT LENGTH(MT.Name) INTO MT_NAME_LENGTH FROM MediaType MT --SELECT LENGTH OF THE NAME FOR THE MEDIATYPE WITH THE MATCHING ID
    WHERE MT.MediaTypeId = MT_ID;
    RETURN MT_NAME_LENGTH;
END;

BEGIN
    DBMS_OUTPUT.PUT_LINE('MT NAME LENGTH '||NAME_LENGTH(1)); --CALL THE NAME_LENGTH FUNCTION AND PRINT TO DBMS OUTPUT
END;

--3.2 System Defined Aggregate Functions
--Create a function that returns the average total of all invoices
CREATE OR REPLACE FUNCTION AVG_INVOICE_TOTAL --NO IN PARAMETERS NEEDED
RETURN NUMBER
IS AVG_TOTAL NUMBER; --RETURN NUMBER
BEGIN
    SELECT AVG(Total) INTO AVG_TOTAL FROM Invoice; --SELECT AVG(TOTAL) FOR ALL INVOICES
    RETURN AVG_TOTAL;
END;

BEGIN --CALL THE AVG_INVOICE FUNCTION AND PRINT RECULT TO DBMS OUTPUT
    DBMS_OUTPUT.PUT_LINE('AVERAGE TOTAL OF ALL INVOICES: '|| AVG_INVOICE_TOTAL()); 
END;

--Create a function that returns the most expensive track
CREATE OR REPLACE FUNCTION MOST_EXPENSIVE --NO IN PARAMETERS NEEDED
RETURN VARCHAR2
IS PRICEY VARCHAR2(200 BYTE);
BEGIN
    SELECT NAME INTO PRICEY FROM Track --SELECT THE NAME
    WHERE UnitPrice = (SELECT MAX(UnitPrice) FROM TRACK); -- UNCORRELATED NESTED QUERY TO SELECT MAX PRICE
    RETURN PRICEY;
END;

BEGIN --THEY'RE ALL THE SAME PRICE SO THIS DOESN'T WORK, NEED TO FIX
...
IS AVG_PRICE NUMBER;
BEGIN
    SELECT AVG(UNITPRICE) INTO AVG_PRICE FROM INVOICELINE; --SELECT AVERAGE PRICE
    RETURN AVG_PRICE;
END;

BEGIN --CALL INVOICELINE_AVG AND PRINT RESULT TO DBMS OUTPUT
...
IS S SYS_REFCURSOR; --RETURN A CURSOR TO THE QUERY RESULTSET
BEGIN
    OPEN S FOR
...
DECLARE --DELCARE CURSOR AND VARIABLES TO RETRIEVE THE RESULT SET
EMP SYS_REFCURSOR;
...
BEGIN
    EMP := EMP_BORN_AFTER_1968(); --CALL THE FUNCTION
    LOOP --LOOP TO PRINT THE RESULTS
...
IS
BEGIN
    OPEN S FOR --RETURN A CURSOR TO THE RESULT SET
    SELECT FIRSTNAME, LASTNAME FROM EMPLOYEE;
END;

DECLARE --DECLARE VARIABLES TO HOLD THE RESULTS
S SYS_REFCURSOR;
EMP_FNAME EMPLOYEE.FIRSTNAME%TYPE;
EMP_LNAME EMPLOYEE.LASTNAME%TYPE;

BEGIN
    EMPLOYEE_NAMES(S); --CALL FUNCTION AND SEND IT THE CURSOR
    LOOP --LOOP TO PRINT RESULTS
...
(E_ID IN NUMBER, NEW_STREET IN VARCHAR2, NEW_CITY IN VARCHAR2, NEW_STATE VARCHAR2) --IN PARAMETERS FOR POSSIBLE UPDATE FIELS
IS
BEGIN 
    UPDATE EMPLOYEE SET ADDRESS=NEW_STREET,CITY=NEW_CITY,STATE=NEW_STATE --PERFORM UPDATE
...
END;
--CHECK THE THE UPDATE COMPLETE SUCCESSFULLY
...
IS
BEGIN
    OPEN S FOR --RETURN A CURSOR TO THE RESULTS
    SELECT E1.EMPLOYEEID,E1.FIRSTNAME,E1.LASTNAME FROM EMPLOYEE E1 --PERFORM A SELF JOIN
...
IS
BEGIN
    SELECT FIRSTNAME,LASTNAME,COMPANY INTO C_FNAME,C_LNAME,C_COMPANY --RETRIEVE THE VALUES FOR THE CUSTOMER
...
BEGIN
    CUSTOMER_INFO(5,C_FNAME,C_LNAME,C_COMPANY); --CALL PROCEDURE AND DISPLAY RESULTS
...
DROP CONSTRAINT FK_INVOICELINEINVOICEID; --REMOVE THE FOREIGN KEY CONSTRAINT

ALTER TABLE INVOICELINE
ADD CONSTRAINT FK_INVOICELINEINVOICEID FOREIGN KEY (INVOICEID) --REMAKE THE FOREIGN KEY WITH A DELETE BEHAVIOR
REFERENCES INVOICE(INVOICEID) ON DELETE CASCADE;

CREATE OR REPLACE PROCEDURE INVOICE_DELETE (I_ID IN NUMBER) --PERFORM TRANSACTION IN A QUERY
IS
BEGIN
    SET TRANSACTION ISOLATION LEVEL SERIALIZABLE; --SET ISOLATION LEVEL
    DELETE INVOICE WHERE INVOICEID = I_ID;
    COMMIT; --DELETE IS FINAL
    
    EXCEPTION --IF THE RECORD IS NOT FOUND, THROW AN EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    INVOICE_DELETE(125); --CALL PRODECURE TO PERFORM THE TRANSACTION
END;
...
BEGIN
    ...
    COMMIT; --PEFORM INSERT AND COMMIT
    
    EXCEPTION --THROW AND EXCEPTION AND ROOLBACK IF THE INSERT IS INVALID
    WHEN OTHERS THEN
