...
CREATE OR REPLACE FUNCTION USER_LOGIN_INFO(
U_NAME IN VARCHAR2, U_PASSWORD IN VARCHAR2)
RETURN VARCHAR2
AS 
REGISTER_USER_LOGINID NUMBER;
BEGIN 
    SELECT USER_ID INTO REGISTER_USER_LOGINID FROM USERS
    WHERE USERNAME = U_NAME AND USER_PASSWORD = U_PASSWORD; 
    IF REGISTER_USER_LOGINID = 0 THEN 
        RETURN 'WRONG USERNAME OR PASSWOR!';
    ELSE IF REGISTER_USER_LOGINID > 1 THEN 
