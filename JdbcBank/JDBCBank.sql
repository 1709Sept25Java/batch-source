...
(U_ID IN INTEGER, PASS OUT INTEGER)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE VIEW_USERS (A_ID IN NUMBER, S OUT SYS_REFCURSOR)
IS
BEGIN
    OPEN S FOR
...
--STORED PROCEDURE TO UPDATE USER ACCOUNT
CREATE OR REPLACE PROCEDURE UPDATE_USER (U_ID IN INTEGER, ADM_RIGHT IN VARCHAR2, FOUND OUT INTEGER)
IS
BEGIN
    SELECT COUNT(USER_ID) INTO FOUND FROM USERS WHERE USER_ID = U_ID;
    IF FOUND = 1 THEN
        UPDATE USERS SET IS_ADMIN = ADM_RIGHT WHERE USER_ID = U_ID;
        COMMIT;
    ELSE
        FOUND := -1;
    END IF;
...
--PROCEDURE TO VIEW ACCOUNTS
CREATE OR REPLACE PROCEDURE GET_ACCOUNTS (U_ID IN NUMBER, S OUT SYS_REFCURSOR)
IS
BEGIN
    OPEN S FOR
...
CREATE OR REPLACE PROCEDURE INSERT_ACCOUNT (U_ID IN NUMBER, PASS OUT INTEGER)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE DELETE_ACCOUNT (U_ID IN INTEGER, BA_ID IN INTEGER, PASS OUT INTEGER)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE ACCOUNT_WITHDRAW (BA_ID IN INTEGER, AMT IN INTEGER, NEW_BAL OUT INTEGER)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE ACCOUNT_DEPOSIT (BA_ID IN INTEGER, AMT IN INTEGER, NEW_BAL OUT INTEGER)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE VIEW_USER_HIST (U_ID IN NUMBER, S OUT SYS_REFCURSOR)
IS
BEGIN
