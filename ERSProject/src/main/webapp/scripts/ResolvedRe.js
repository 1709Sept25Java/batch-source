function getResolved(xhr){
	...
	for(i = 0; i < resolved.length; i++){
		...
		+resolved[i].submitted+"</td><td>"+resolved[i].resolved+"</td><td>"+resolved[i].author+"</td><td>"+resolved[i].resolver+"</td>"
				+"<td>"+resolved[i].type1+"</td><td>"+resolved[i].status1+"</td></tr>";
	}
}
...
window.onload = function(){
	sendAJAX("http://localhost:8082/ERSProject/resolved", getResolved);
}