...
/*2.1 Select statements: 1) gets all from Employee table, 
...
/*2.2 Order by: 1) all from employee descending
...
/*2.5 Like... gets everything in invoice with a billingaddress containing T*/
SELECT * from INVOICE where BILLINGADDRESS like 'T%';

/*2.6 Between  1)invoice with total between 15 and 50
...
4.1 creates procedure on getting the firstname and lastname of the employee*/
...
--6.1 After inserting into employee, console outputs Inserting into Employee
...
END;
/
--6.2 After inserting into album, console outputs Inserting into Album and
Updates the Album
...
END;
/
--6.3 After Delete from customera, console outputs delete from customer
CREATE OR REPLACE TRIGGER t3
BEFORE DELETE ON CUSTOMERS
    FOR EACH ROW
BEGIN
        DBMS_OUTPUT.PUT_LINE('Delete from customers');
END;
/

/*7.0 Joins*/
/*7.1 Inner joins table with cutomer's first and lastname, with invoiceid
based on customerid and invoiceid*/
SELECT CUSTOMER.FIRSTNAME, CUSTOMER.LASTNAME, INVOICE.INVOICEID
FROM CUSTOMER
INNER JOIN INVOICE ON CUSTOMER.CUSTOMERID = INVOICE.INVOICEID;
/*7.2 Outer joins table with cutomer's first and lastname, with invoice's id and total
based on customerid and invoiceid*/
...
/*7.3 Right joins album title and artist name based on albumid and artistid*/
SELECT ALBUM.TITLE, ARTIST.NAME 
FROM ALBUM
RIGHT JOIN ARTIST ON ALBUM.ALBUMID = ARTIST.ARTISTID;
/*7.4 Cross join album title and artist name based on albumid and artistid
ordered by artist name ...note that use where instead of on*/
SELECT ALBUM.TITLE, ARTIST.NAME 
FROM ALBUM
CROSS JOIN ARTIST Where ALBUM.ALBUMID = ARTIST.ARTISTID
ORDER BY ARTIST.NAME;
/*7.5 Self joins firstname A and B of employees' based on reportsto
