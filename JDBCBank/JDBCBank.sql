...
--LOGIN 
CREATE OR REPLACE FUNCTION LOGIN(U IN VARCHAR2, P IN VARCHAR2) 
RETURN NUMBER
IS
Z NUMBER;
BEGIN
    SELECT USERID INTO Z FROM USERS 
    WHERE USERNAME=U AND PASSKEY=P;
    RETURN Z;
END;
...
CREATE OR REPLACE PROCEDURE REGISTER_USER(N IN VARCHAR2, P IN VARCHAR2) 
IS 
U_EXISTS NUMBER;
BEGIN 
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
SELECT COUNT(U.USERNAME) INTO U_EXISTS FROM USERS U
WHERE U.USERNAME = N;
    IF U_EXISTS > 0 THEN
    ...
    ELSE
        ...
        VALUES(N,P,'C');
        DBMS_OUTPUT.PUT_LINE('CREATED USER: '||N);
    END IF;
    COMMIT;
    EXCEPTION
    WHEN OTHERS THEN
...
--VIEW USERS (ADMIN)
CREATE OR REPLACE PROCEDURE VIEW_USERS(A IN NUMBER, S OUT SYS_REFCURSOR) 
IS 
A_EXISTS NUMBER;
BEGIN 
    SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
    SELECT COUNT(U.USERID) INTO A_EXISTS FROM USERS U
    WHERE U.USERID=A AND U.USERTYPE='A'; 
    IF A_EXISTS > 0 THEN
        OPEN S FOR
        SELECT USERID, USERNAME, USERTYPE FROM USERS;
        DBMS_OUTPUT.PUT_LINE('GET USERS');
    ELSE 
        DBMS_OUTPUT.PUT_LINE('NOT ADMIN');
    END IF;
COMMIT;
EXCEPTION
WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('FAILED TO RETRIEVE USERS');
...
--CREATE USER (ADMIN)
CREATE OR REPLACE PROCEDURE CREATE_USER(A IN NUMBER, N IN VARCHAR2, P IN VARCHAR2, T IN VARCHAR2)
IS 
A_EXISTS NUMBER;
BEGIN 
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
SELECT COUNT(U.USERID) INTO A_EXISTS FROM USERS U
    ...
    IF A_EXISTS > 0 THEN
        ...
        VALUES(N,P,T);
    ELSE
        DBMS_OUTPUT.PUT_LINE('Do not have admin status!');
    END IF;
    COMMIT;
    EXCEPTION
    WHEN OTHERS THEN
...
--UPDATE USER 
CREATE OR REPLACE PROCEDURE UPDATE_USER(A IN NUMBER, I IN NUMBER, N IN VARCHAR2, P IN VARCHAR2, T IN VARCHAR2)
IS 
A_EXISTS NUMBER;
BEGIN 
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
SELECT COUNT(U.USERID) INTO A_EXISTS FROM USERS U
    ...
    IF A_EXISTS > 0 THEN
        UPDATE USERS
        SET USERNAME = N, PASSKEY = P, USERTYPE = T
        WHERE USERID = I;
    ELSE
        DBMS_OUTPUT.PUT_LINE('Do not have admin status!');
    END IF;
...
--DELETE USER (ADMIN)
CREATE OR REPLACE PROCEDURE DELETE_USER(A IN NUMBER, N IN NUMBER) 
IS 
A_EXISTS NUMBER;
BEGIN 
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
...
BEGIN 
    ...
    IF A_EXISTS > 0 THEN
        ...
        WHERE A.ACCOUNTUSER = UI;
        DBMS_OUTPUT.PUT_LINE('GET ACCOUNTS');
    ELSE 
...
CREATE OR REPLACE PROCEDURE VIEW_TRANSACTIONS(UI IN NUMBER, S OUT SYS_REFCURSOR)
IS 
U_EXISTS NUMBER;
BEGIN 
    SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
    SELECT COUNT(U.USERID) INTO U_EXISTS FROM USERS U WHERE U.USERID = UI;
    IF U_EXISTS > 0 THEN
        OPEN S FOR         
            SELECT T.* FROM TRANSACTIONS T WHERE 
                T.TRANSACTIONACCOUNT IN (SELECT ACCOUNTID FROM ACCOUNTS WHERE ACCOUNTUSER = UI);
    ELSE 
...
EXCEPTION
WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('FAILED TO RETRIEVE TRANSACTION');
ROLLBACK;
END;
